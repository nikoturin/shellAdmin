...
function aviator_queryComp()
{
 ...
 query="set isolation dirty read;
	...
	select x.xx,x.xx,xx,xx, a.* from xxxx A
	where xxxx = '$gl_ICAO$lc_flightNo'
	and xxxx     = '$month/$day/$year'
	and xxxx  = '$lc_flightDepa'
	and xxxx    = '$lc_flightArr'"
 ...
 do 
     if [[ $dataSeats -eq $(echo ${lc_arrSeats[$cnt_arrSeats]}|bc) ]]
      then
         echo -e "$day/$month/$year\t\t$gl_ICAO$lc_flightNo\t\t\tOk\t\t$dataSeats\t\t${lc_arrSeats[$cnt_arrSeats]}\t\t\t${lc_arrClass[$cnt_arrSeats]}"
      else
         echo -e "$day/$month/$year\t\t$gl_ICAO$lc_flightNo\t\t\tNot Match\t$dataSeats\t\t${lc_arrSeats[$cnt_arrSeats]}\t\t\t${lc_arrClass[$cnt_arrSeats]}"
     fi
...
function aviator_date()
{
  ...
  echo -e ">------------------------------------------------------------------------------------------------------------------<"
  echo -e ">------------------------------------------AVIATOR DATA MATCH------------------------------------------------------<"	
  echo -e "--------------------------------------------------------------------------------------------------------------------"
  echo -e " Date(d/m/y)\t|\tFlight No.\t|\tStatus\t|\tSeatDB\t|\tSeatAviator\t|\tClass\t|"
  echo -e "--------------------------------------------------------------------------------------------------------------------"
